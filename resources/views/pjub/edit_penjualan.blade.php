...
@section('content')
    ...
    <form action="/pjub/penjualan/{{ $penjualans->penjualan_id }}/update" method="POST" enctype="multipart/form-data">
		@csrf
    <div class="card-body">
      <div class="row">
            ...
            <select name="siklus_id" class="form-control" id="exampleControlInput1" required oninvalid="this.setCustomValidity('Data tidak boleh kosong!')" oninput="setCustomValidity('')">
                @foreach ($sikluses as $siklus)
                    <option value="{{ $siklus->siklus_id }}"> {{ $siklus->nama_siklus == $siklus->siklus_id ? 'selected' : '' }} 
                        {{ $siklus->nama_siklus }} - {{ $siklus->nama_farm }} - {{ $siklus->nama }}
                    </option>
                @endforeach
            </select>
        </div>
        ...
        <div class="col-md-6" >
            ...
            <a href="/pjub/penjualan" class="btn btn-danger my-1 ml-2">Kembali</a>
        </div>
      </div>
    </div>
	</form>
